...
class Field:
    ...
    def step(self, InBody, dt):
        ...
        Параметры:
            InBody - Список, в котором каждой точке присвоен номер тела, в которой она состоит,
                            либо -1, если она свободная
            dt - дискретный шаг времени
        '''
...
class body_field:
    ...
    def speed_points(self, dt):
        ...
        for body in self.bodies:
            ...
            for p in body.points:
                s = p.coords - body.center
                p.speed = body.speed + s.rotated(np.pi / 2) * (body.omega)
                #print((s.rotated(np.pi/2)*(body.omega))//s)
